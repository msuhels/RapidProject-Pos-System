...
import { useOrderLabels, type OrderLabel } from '../hooks/useOrderLabels';
import { useAuthStore } from '@/core/store/authStore';
import { useModules } from '@/core/hooks/useModules';

interface OrderLabelsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function OrderLabelsDialog({ open, onOpenChange }: OrderLabelsDialogProps) {
  const { labels, loading, refetch } = useOrderLabels();
  const { accessToken } = useAuthStore();
  const { findModuleByCode } = useModules();
  const [editingId, setEditingId] = useState<string | null>(null);
  const [form, setForm] = useState({ name: '', color: '#3b82f6' });

  const module = findModuleByCode('orders');
  ...
  const handleUpdate = async (label: OrderLabel) => {
  ...
  const handleDelete = async (label: OrderLabel) => {
  ...
  const startEdit = (label: OrderLabel) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Manage Order Labels</DialogTitle>
        </DialogHeader>
            ...
            <div className="space-y-2">
              {labels.map((label) => (
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <Button variant="ghost" size="sm" onClick={() => startEdit(label)}>
