...
import { useCartLabels, type CartLabel } from '../hooks/useCartLabels';
import { useAuthStore } from '@/core/store/authStore';
import { useModules } from '@/core/hooks/useModules';

interface CartLabelsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function CartLabelsDialog({ open, onOpenChange }: CartLabelsDialogProps) {
  const { labels, loading, refetch } = useCartLabels();
  ...
  const handleUpdate = async (label: CartLabel) => {
  ...
  const handleDelete = async (label: CartLabel) => {
  ...
  const startEdit = (label: CartLabel) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Manage Cart Labels</DialogTitle>
        </DialogHeader>
