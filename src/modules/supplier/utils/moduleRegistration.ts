...
import { SUPPLIER_PERMISSIONS } from '../config/permissions.config';
import { SUPPLIER_FIELDS } from '../config/fields.config';

const MODULE_CODE = 'SUPPLIER';

/**
 * Register permissions for the supplier module
 */
export async function registerSupplierPermissions(userId?: string): Promise<void> {
  ...
  const permissionsToInsert = SUPPLIER_PERMISSIONS.filter(
...
/**
 * Register system fields for the supplier module
 */
export async function registerSupplierFields(userId?: string): Promise<void> {
  // Try both uppercase and lowercase module codes
  const moduleRecord = await db
    .select()
    .from(modules)
  ...
  const fieldsToInsert = SUPPLIER_FIELDS.filter((field) => !existingFieldCodes.has(field.code)).map(
    (field) => ({
      moduleId,
      name: field.name,
      code: field.code,
 ...
 */
export async function registerSupplierModule(userId?: string): Promise<void> {
  await registerSupplierPermissions(userId);
  await registerSupplierFields(userId);
}
