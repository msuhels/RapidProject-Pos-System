...
import { useInventoryLabels, type InventoryLabel } from '../hooks/useInventoryLabels';
import { useAuthStore } from '@/core/store/authStore';
import { useModules } from '@/core/hooks/useModules';

interface InventoryLabelsDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function InventoryLabelsDialog({ open, onOpenChange }: InventoryLabelsDialogProps) {
  const { labels, loading, refetch } = useInventoryLabels();
  ...
  const handleUpdate = async (label: InventoryLabel) => {
  ...
  const handleDelete = async (label: InventoryLabel) => {
  ...
  const startEdit = (label: InventoryLabel) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Manage Inventory Labels</DialogTitle>
        </DialogHeader>
