...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check permission
    const hasPermission = await userHasPermission(userId, 'customer_management:import') ||
    ...
    if (moduleRecord.length > 0) {
      ...
      fields.forEach((field) => {
        const headerIndex = headers.findIndex((h) => h.toLowerCase() === (field.label || field.name).toLowerCase());
        if (headerIndex !== -1) {
      ...
      try {
        ...
        for (const [code, indexStr] of systemFieldMap.entries()) {
          ...
          if (index < row.length) {
            ...
            if (value) {
              if (code === 'total_purchases' || code === 'outstanding_balance') {
                customerData[code] = parseFloat(value) || 0;
              } else if (code === 'is_active') {
                customerData[code] = value.toLowerCase() === 'yes' || value.toLowerCase() === 'true';
              } else {
        ...
        for (const [code, { index, fieldType }] of customFieldMap.entries()) {
          if (index < row.length) {
            ...
            if (value) {
              if (fieldType === 'number') {
                customerData.customFields[code] = parseFloat(value) || 0;
              } else if (fieldType === 'boolean') {
                customerData.customFields[code] = value.toLowerCase() === 'yes' || value.toLowerCase() === 'true';
              } else {
                customerData.customFields[code] = value;
              }
            }
          }
        }
