...
/**
 * Register permissions for the customer_management module
 */
export async function registerCustomerPermissions(userId?: string): Promise<void> {
...
/**
 * Register system fields for the customer_management module
 */
export async function registerCustomerFields(userId?: string): Promise<void> {
  // Try both uppercase and lowercase module codes
  const moduleRecord = await db
    .select()
    .from(modules)
  ...
  const fieldsToInsert = CUSTOMER_FIELDS.filter((field) => !existingFieldCodes.has(field.code)).map(
    (field) => ({
      moduleId,
      name: field.name,
      code: field.code,
 ...
 */
export async function registerCustomerModule(userId?: string): Promise<void> {
  await registerCustomerPermissions(userId);
  await registerCustomerFields(userId);
}
